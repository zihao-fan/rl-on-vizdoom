...
class Agent(object):
    def __init__(self, input_size, act_space, config):
        ...
        self.config = config
        print('Agent.input_size', self.input_size, 'Agent.act_space', self.act_space)

        # Shared network.
        net = mx.sym.Variable('data')
        net = mx.sym.Convolution(data=net, name='conv1', kernel=(8, 8), stride=(4,4), num_filter=16)
        ...
        policy_fc = mx.sym.FullyConnected(
            data=net, name='policy_fc', num_hidden=act_space, no_bias=False)
    ...
    def train_step(self, env_xs, env_as, env_rs, env_vs, paralell_num):
        ...
        v_grads = mx.nd.array(self.config.vf_wt * neg_advs_v[:, np.newaxis],
                              ctx=self.ctx)
        # print('out grads, neg_advs.mean=', np.mean(neg_advs.asnumpy()), 'v_grads=', np.mean(v_grads.asnumpy()))
